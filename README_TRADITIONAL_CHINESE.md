...
1. Fork這個repo
1. 通過運行 `yarn` 在本地安裝依賴項
1. 進行更改
1. 確保使用 `yarn build` 進行打包
1. 使用 `yarn test` 運行測試（你添加了測試，對吧？）
1. 在您運行的代碼或者我們的演示項目中測試您的更改：運行 [`yarn link`](https://classic.yarnpkg.com/en/docs/cli/link)
1. 確保代碼覆蓋率與更改前相同或更高
