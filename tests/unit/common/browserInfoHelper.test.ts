...
import globalsUtil, {ExtendedNavigator} from '../../../src/js/common/globalsUtil';
import {
...
describe('BrowserInfoHelper', () => {
    ...
    describe('getBrowserDoNotTrackStatus', () => {
        ...
        getBrowserDoNotTrackStatusTests.forEach(({test, mockWindow = {}, mockNavigator = {}, expected}) => {
            it(test, () => {
                spies.stub(globalsUtil, 'getWindow').returns(mockWindow as typeof window);
                spies.stub(globalsUtil, 'getNavigator').returns(mockNavigator as ExtendedNavigator);
    ...
    describe('getDeviceLocalIPAsString', () => {
        let webRtcConnectionStub: sinon.SinonStub<any[], RTCPeerConnection | undefined>;
        beforeEach(() => {
            webRtcConnectionStub = spies.stub(globalsUtil, 'getWebRTCConnection') as any;
            resetDeviceIpString();
        });
        ...
        it('RTCPeerConnection throws unexpected error', async () => {
            ...
            try {
                ...
                expect(error.toString()).toEqual('Error: MockErrorRTCPeerConnection error thrown');
                return;
            }
            throw new Error('Expected an error to be thrown, but it was not');
        });
    ...
    it('getMTDTaxReturnWithParameter', async () => {
        ...
        spies.stub(globalsUtil, 'getWebRTCConnection').returns(MockRTCPeerConnection as any);
