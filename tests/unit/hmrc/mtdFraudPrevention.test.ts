...
import {MockRTCPeerConnection, setAdditionalCandidateString, resetCandidateString} from '../mock/MockRTCPeerConnection';
...
describe('FraudPreventionHeaders', () => {
    ...
    it('getFraudPreventionHeaders with no errors', async () => {
        ...
        expect(headers.get('Gov-Client-Timezone')).toBe(`UTC${moment().format('Z')}`);
        expect(headers.get('Gov-Client-Screens')).toBe('width=1019&height=1021&scaling-factor=2&colour-depth=17');
        expect(headers.get('Gov-Client-Window-Size')).toBe('width=1009&height=1013');
        expect(headers.get('Gov-Client-Browser-Plugins')).toBe('ABC%20Plugin,XYZ%20Plugin');
